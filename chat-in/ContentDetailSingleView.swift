...
struct ContentDetailSingleView: View {
    var chatGroup: String
    var chatHeader: String
    var lastChat: String
    @StateObject private var messageStore = MessageStore()
    @StateObject private var userStore = UserStore()
    @StateObject private var userStore2 = UserStore()
    ...
    @State private var headerid2: String = ""
        ...
        GeometryReader { geometry in
            VStack {
                            ...
                            VStack {
                                ...
                                HStack {
                                    Text(message.content).font(.title3)
                                        ...
                                        .padding()
                                    AsyncMiniUserView(imageUrl: URL(string: "https://\(serverurl)/photos/\(userStore2.user.photo)")!)
                                }
                                .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .trailing)
                                .background(.tertiary)
                            }
                ...
                HStack {
                    ...
                    .padding(.trailing)
                    .disabled(chatgptkey.isEmpty || message.isEmpty)
    ...
    private func updateHeader(completion: @escaping (Result<String, Error>) -> Void) {
        guard let url = URL(string: "https://\(serverurl)/headers/\(chatHeader)") else { return }
    ...
    private func updateHeader2(completion: @escaping (Result<String, Error>) -> Void) {
        guard let url = URL(string: "https://\(serverurl)/headers/\(headerid2)") else { return }
    ...
    func updateChatGPTHeader() {
        let url = URL(string: "https://\(serverurl)/headers/\(chatHeader)")!
        var request = URLRequest(url: url)
        request.httpMethod = "PATCH"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue("Bearer \(usertoken)", forHTTPHeaderField: "Authorization")
    ...
    func updateChatGPTHeader2() {
        let url = URL(string: "https://\(serverurl)/headers/\(headerid2)")!
        var request = URLRequest(url: url)
        request.httpMethod = "PATCH"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue("Bearer \(usertoken)", forHTTPHeaderField: "Authorization")
    ...
    func fetchSenderData() {
        guard let url = URL(string: "https://\(serverurl)/headers/by-sender?userId=\(userid)&otherId=\(chatGroup)") else { return }
    ...
    func fetchRecipientData() {
        guard let url = URL(string: "https://\(serverurl)/headers/by-recipient?userId=\(userid)&otherId=\(chatGroup)") else { return }
    ...
    func fetchAPIData3() {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            do {
                let apiResponse = try JSONDecoder().decode(User.self, from: data)
                DispatchQueue.main.async {
                    userStore2.updateUser(from: apiResponse)
                }
            } catch {
                print("Error decoding JSON: \(error.localizedDescription)")
            }
...
#Preview {
    ContentDetailSingleView(chatGroup: "66ac483a9f7041b43eb34f79", chatHeader: "66b3256cb34fc72bae103250", lastChat: "Sender")
}
